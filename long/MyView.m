...
- (void)mouseDown:(NSEvent *)e
{
	...
	if(dic)
	{
		...
		sindex=[[dic objectForKey:@"i"] intValue];
		[app selectTableRowAtIndex:sindex];
...
-(void)mouseDragged:(NSEvent*)e
{
	...
	if(rindex==0)
	{
		NSRect			r=[self frame];
		float			x,y;
		NSMutableArray	*age=[data objectForKey:@"Age"];
		NSMutableArray	*var=[[[data objectForKey:@"Variables"] objectAtIndex:vindex] objectForKey:@"vals"];
		
		x=m.x*(tmax-tmin)/r.size.width+tmin;
		y=m.y*(vmax-vmin)/r.size.height+vmin;
		
		[age replaceObjectAtIndex:sindex withObject:[NSString stringWithFormat:@"%g",x]];
		[var replaceObjectAtIndex:sindex withObject:[NSString stringWithFormat:@"%g",y]];
	}
	else
	if(rindex==1)
